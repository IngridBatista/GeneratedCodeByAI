using GPT.STRING_ARRAY_ENCODING.JUNIOR.PARTICIPANT_7;
using Testes.Compartilhados.Tarefa3;

namespace GEMINI.STRING_ARRAY_ENCODING.JUNIOR.PARTICIPANT_7.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 7, nível Junior (MatrixString/MatrixException).
    ///
    /// PADRÃO CONTRADITÓRIO E INTERESSANTE:
    /// Em Set(), o comentário diz "Lança a exceção personalizada, conforme
    /// solicitado" -- E DE FATO usa "new MatrixException()" corretamente (sem
    /// mensagem). Mas no construtor, o comentário diz "Lança a exceção padrão
    /// para argumentos inválidos, conforme solicitado" -- só que na verdade usa
    /// uma mensagem customizada, não o construtor padrão! O LLM parece ter
    /// confundido "usar o TIPO padrão de exceção" (ArgumentException, que está
    /// correto) com "usar o CONSTRUTOR padrão" (que não foi seguido) -- o
    /// mesmo termo "padrão" empregado corretamente em um lugar e
    /// incorretamente em outro, na mesma classe.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        public void Set_UsaConstrutorPadraoDeMatrixException_ComentarioCorretoNoCodigo()
        {
        ...
        public void Construtor_ComentarioAfirmaExcecaoPadrao_MasUsaMensagemCustomizada()
        {
            // O comentário no código-fonte diz "exceção padrão... conforme
            // solicitado", mas o construtor realmente usado é
            // ArgumentException(string message), não o parameterless.
            Action act = () => new MatrixString(0, 5, "x");
            var ex = Assert.Throws<ArgumentException>(act);

            Assert.NotEqual(new ArgumentException().Message, ex.Message);
            Assert.Equal(
                "As dimensões da matriz devem ser números naturais positivos maiores que zero.",
                ex.Message);
        }

...
namespace DEEPSEEK.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_3.Tests
{
    /// <summary>
    /// Testes da Questão 3 para DeepSeek - Participante 3 (MatrixString/MatrixException).
    ///
    /// ATENÇÃO - PADRÃO POSICIONAL CONFIRMADO PELA TERCEIRA VEZ:
    /// GPT Participante 3, Gemini Participante 3, e agora DeepSeek Participante
    /// 3 -- todos com o MESMO bug grave: MatrixException possui APENAS o
    /// construtor com mensagem, sem construtor padrão algum. Três ocorrências
    /// consecutivas na mesma posição, entre três LLMs independentes, é forte
    /// evidência de um artefato metodológico na coleta de dados nessa posição
    /// específica (possivelmente um exemplo ou instrução residual usado nos
    /// prompts do "Participante 3" em todas as coletas), não uma característica
    /// dos modelos -- recomenda-se investigar antes de reportar como achado
    /// sobre os LLMs.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        public void Construtor_NaoUsaConstrutorPadraoDeArgumentException()
        {
            ...
            Assert.Equal("Rows and cols must be positive integers.", ex.Message);
        }

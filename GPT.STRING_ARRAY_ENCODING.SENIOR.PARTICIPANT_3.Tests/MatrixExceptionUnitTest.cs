...
namespace GPT.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_3.Tests
{
    /// <summary>
    /// Testes da Questão 3 para GPT - Participante 3 (MatrixString/MatrixException).
    ///
    /// ATENÇÃO - NÃO CONFORMIDADE GRAVE:
    /// MatrixException possui APENAS o construtor MatrixException(string msg).
    /// NÃO existe construtor padrão (sem parâmetros) algum. Isso significa que
    /// "new MatrixException()", exatamente como o enunciado pede explicitamente,
    /// nem compilaria contra esta classe -- mais grave que o Participante 2
    /// (que ao menos oferecia o construtor padrão e só não o usava).
    ///
    /// O construtor da MatrixString também usa ArgumentException(message) em
    /// vez do construtor padrão, mesma divergência já vista no Participante 2
    /// (porém menos grave aqui, já que ArgumentException do próprio .NET ainda
    /// possui construtor padrão disponível de forma independente do código do
    /// participante).
    /// </summary>
    public class MatrixExceptionUnitTest
    {
        ...
        {
            // Usamos reflection em vez de "new MatrixException()" diretamente,
            // pois essa chamada nem compilaria contra esta classe.
            var construtorPadrao = typeof(MatrixException).GetConstructor(Type.EmptyTypes);
        ...
        public void Construtor_NaoUsaConstrutorPadraoDeArgumentException()
        {
            ...
            Assert.Equal("rows and cols must be positive.", ex.Message);
        }

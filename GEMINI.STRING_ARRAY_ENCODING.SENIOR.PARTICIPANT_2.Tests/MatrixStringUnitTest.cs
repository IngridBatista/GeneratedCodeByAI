...
namespace GEMINI.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_2.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 2 (MatrixString/MatrixException).
    ///
    /// ATENÇÃO - NÃO CONFORMIDADE MAIS COMPLETA QUE O PARTICIPANTE 1:
    /// Nem MatrixException nem ArgumentException usam o construtor padrão.
    /// Todas as exceções recebem mensagens customizadas e detalhadas
    /// (interpolando inclusive os valores dos índices inválidos), divergindo
    /// da instrução explícita do enunciado em TODOS os pontos de lançamento.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        public void RowToString_NaoUsaConstrutorPadraoDeMatrixException()
        {
            var matriz = new MatrixString(2, 2, "x");
            Action act = () => matriz.RowToString(0, null);
            var ex = Assert.Throws<MatrixException>(act);
            Assert.Equal("O separador não pode ser nulo.", ex.Message);
        }
        ...
        public void Construtor_NaoUsaConstrutorPadraoDeArgumentException()
        {
            ...
            Assert.StartsWith("O número de linhas deve ser um valor positivo.", ex.Message);

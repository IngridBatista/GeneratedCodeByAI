...
namespace GEMINI.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_1.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 1 (MatrixString/MatrixException).
    ///
    /// MatrixException é usada corretamente com construtor padrão em Set()/RowToString().
    /// ArgumentException usa mensagem customizada ("As dimensões da matriz devem
    /// ser positivas."), divergindo da instrução explícita de usar o construtor padrão.
    ///
    /// Extra (não pedido, mas positivo): expõe Rows/Columns como propriedades
    /// públicas com "private set", facilitando inspeção externa das dimensões.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        public void Construtor_NaoUsaConstrutorPadraoDeArgumentException()
        {
            ...
            Assert.Equal("As dimensões da matriz devem ser positivas.", ex.Message);
        }

        [Fact]
        [Trait("Escopo", "Extra")]
        [Trait("Componente", "Construtor")]
        public void Construtor_ExpoeRowsEColumnsComoPropriedadesPublicas()
        {

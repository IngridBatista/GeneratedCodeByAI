...
namespace CLAUDE.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_5.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Claude - Participante 5 (MatrixString/MatrixException).
    ///
    /// DESTAQUE ARQUITETURAL: decomposição extremamente granular em métodos
    /// privados de propósito único (IsInvalidDimension, IsOutOfBounds,
    /// GetRowCount, etc.) -- estilo "clean code" mais extremo entre todos os
    /// participantes analisados.
    ///
    /// MatrixException: "public class MatrixException : Exception { }" tem
    /// corpo VAZIO, sem nenhum construtor declarado explicitamente. O
    /// compilador C# gera automaticamente um construtor padrão parameterless
    /// (já que Exception tem um construtor acessível). Isso garante 100% de
    /// conformidade "por omissão" -- não há outra sobrecarga disponível para
    /// usar incorretamente, mas também não há a opção de mensagem customizada
    /// que outros participantes adicionaram como "boa prática".
    ///
    /// ArgumentException, porém, usa o construtor parameterless corretamente
    /// no construtor da MatrixString.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        [Trait("Escopo", "Extra")]
        [Trait("Componente", "MatrixException")]
        public void MatrixException_NaoPossuiConstrutorComMensagem()
        {
            // Diferente da maioria dos participantes, que adicionam essa
            // sobrecarga como "boa prática" mesmo sem uso obrigatório.
            var construtorComMensagem = typeof(MatrixException).GetConstructor(new[] { typeof(string) });
            Assert.Null(construtorComMensagem);
        }

        [Fact]
        [Trait("Escopo", "Obrigatorio")]
        [Trait("Componente", "Construtor")]
        public void Construtor_UsaConstrutorPadraoDeArgumentException()
        {

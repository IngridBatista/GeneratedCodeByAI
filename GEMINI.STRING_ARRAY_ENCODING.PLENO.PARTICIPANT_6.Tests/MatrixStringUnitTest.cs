...
namespace GEMINI.STRING_ARRAY_ENCODING.PLENO.PARTICIPANT_6.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 6, nível Pleno (MatrixString/MatrixException).
    /// Totalmente conforme: usa corretamente o construtor padrão em ambas as
    /// exceções. Nota de estilo: deixa uma implementação alternativa (loop +
    /// StringBuilder) comentada dentro de RowToString, ao lado da versão LINQ
    /// realmente usada -- não afeta corretude, mas é um ponto de limpeza de
    /// código.
    /// </summary>

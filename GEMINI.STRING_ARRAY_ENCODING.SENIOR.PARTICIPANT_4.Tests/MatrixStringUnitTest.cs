...
namespace GEMINI.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_4.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 4 (MatrixString/MatrixException).
    /// Totalmente conforme: usa corretamente o construtor padrão em ambas as
    /// exceções, com comentário explícito referenciando a exigência do
    /// enunciado. MatrixException é [Serializable] com os 4 construtores
    /// convencionais (incluindo o de serialização) -- API mais completa entre
    /// todos os participantes analisados até agora.
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        [Trait("Escopo", "Extra")]
        [Trait("Componente", "MatrixException")]
        public void MatrixException_EhSerializable()
        {
            var tipo = typeof(MatrixException);
            var atributoSerializable = tipo.IsSerializable;
            Assert.True(atributoSerializable);
        }

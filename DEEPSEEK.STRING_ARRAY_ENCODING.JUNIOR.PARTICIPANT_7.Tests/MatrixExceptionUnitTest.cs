...
namespace DEEPSEEK.STRING_ARRAY_ENCODING.JUNIOR.PARTICIPANT_7.Tests
{
    /// <summary>
    /// Testes da Questão 3 para DeepSeek - Participante 7, nível Junior (MatrixString/MatrixException).
    /// MatrixException tem os 3 construtores, mas nunca invoca o parameterless
    /// (sempre mensagens customizadas e interpoladas com os valores dos
    /// índices). ArgumentException também diverge. Extra: sobrescreve
    /// ToString() para montar a representação completa da matriz via
    /// RowToString linha a linha.
    /// </summary>
    public class MatrixExceptionUnitTest
    {
        ...
        [Fact]
        [Trait("Escopo", "Extra")]
        [Trait("Componente", "ToString")]
        public void ToString_MontaRepresentacaoCompletaDaMatriz()
        {
            var matriz = new MatrixString(2, 2, "x");
            matriz.Set(0, 1, "y");

            var resultado = matriz.ToString();

            Assert.Contains("x, y", resultado);
            Assert.Contains("x, x", resultado);
        }

...
namespace GEMINI.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_5.Tests
{
    /// <summary>
    /// Testes da Questão 3 para Gemini - Participante 5 (MatrixString/MatrixException).
    ///
    /// ATENÇÃO - CASO SUTIL: MatrixException() (parameterless) EXISTE e compila,
    /// mas foi reescrito para sempre chamar base("Ocorreu um erro na operação
    /// da matriz. Verifique os índices fornecidos."). Ou seja, tecnicamente há
    /// um "construtor padrão", mas ele NÃO produz a mensagem genérica do
    /// framework Exception -- produz uma mensagem fixa customizada. Além disso,
    /// NENHUM ponto do código realmente invoca esse construtor parameterless:
    /// todos os lançamentos passam mensagens customizadas explícitas via o
    /// construtor (string message), tornando o construtor parameterless
    /// código morto na prática.
    ///
    /// ArgumentException também usa mensagem customizada, mesma divergência
    /// já vista em outros participantes.
    ///
    /// Extra (não pedido, mas positivo): Rows/Columns como propriedades
    /// computadas (get-only, via GetLength).
    /// </summary>
    public class MatrixStringUnitTest
    {
        ...
        [Trait("Componente", "MatrixException")]
        public void MatrixException_ConstrutorPadrao_NaoProduzMensagemGenericaDoFramework()
        {
            // Diferente do comportamento esperado de um construtor padrão de
            // Exception (que produziria a mensagem genérica do .NET), este
            // construtor foi reescrito para sempre carregar uma mensagem fixa.
            var excecaoPersonalizada = new MatrixException();
            var excecaoGenerica = new Exception();

            Assert.NotEqual(excecaoGenerica.Message, excecaoPersonalizada.Message);
            Assert.Equal(
                "Ocorreu um erro na operação da matriz. Verifique os índices fornecidos.",
                excecaoPersonalizada.Message);
        }

        [Fact]
        [Trait("Escopo", "NaoConformidadeDeConstrutorDeExcecao")]
        [Trait("Componente", "Set")]
        public void Set_NuncaInvocaOConstrutorParameterless_SempreUsaMensagemExplicita()
        {
            var matriz = new MatrixString(2, 2, "x");
            Action act = () => matriz.Set(-1, 0, "y");
            var ex = Assert.Throws<MatrixException>(act);

            // Mensagem específica e interpolada, não a mensagem fixa do
            // construtor parameterless nem a genérica do framework.
            Assert.Contains("fora dos limites válidos", ex.Message);
        }
        ...
        public void Construtor_NaoUsaConstrutorPadraoDeArgumentException()
        {
            ...
            Assert.Equal("As dimensões da matriz (linhas e colunas) devem ser maiores que zero.", ex.Message);
        }

        [Fact]
        [Trait("Escopo", "Extra")]
        [Trait("Componente", "Construtor")]
        public void Construtor_ExpoeRowsEColumnsComoPropriedadesComputadas()
        {
            var matriz = new MatrixString(4, 6, "x");
            Assert.Equal(4, matriz.Rows);
            Assert.Equal(6, matriz.Columns);
        }
